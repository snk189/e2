...
const BookingInterface = ({ onLogout, username }) => {
  ...
  const calculateTotalPrice = () => {
    ...
    Object.keys(quantities).forEach(id => {
      ...
      if (item) {
        const price = parseInt(item.price.replace('Γé╣', ''), 10);
      ...
      <main className="max-w-md mx-auto px-4">
        ...
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 mb-6">
          ...
          <div className="flex p-1.5 rounded-2xl mb-4 shadow-inner border border-gray-100 bg-gray-50/80 backdrop-blur">
            ...
            >
              <div className={`flex items-center justify-center w-7 h-7 rounded-full ${orderType === 'dine-in' ? 'bg-white/25 shadow-sm' : 'bg-gray-200'}`}>≡ƒì╜∩╕Å</div> Dine-In
            </button>
            <button
              onClick={() => {
            ...
            >
              <div className={`flex items-center justify-center w-7 h-7 rounded-full ${orderType === 'prebook' ? 'bg-white/25 shadow-sm' : 'bg-gray-200'}`}>≡ƒôà</div> Prebook
            </button>
          </div>
          ...
          <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 mb-6 animate-in slide-in-from-bottom-4 fade-in duration-300">
              ...
              {Object.keys(quantities).map(id => {
                ...
                return (
                  <div key={id} className="flex justify-between items-center text-sm font-medium text-gray-700">
                    ...
                    <span className="shrink-0 font-bold">Γé╣{itemTotal}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="border-t border-dashed border-gray-200 pt-3 space-y-2">
              <div className="flex justify-between items-center text-sm font-bold text-gray-500">
                <span>Subtotal</span>
                <span>Γé╣{calculateTotalPrice()}</span>
              </div>
              ...
              {orderType === 'prebook' && (
                <div className="flex justify-between items-center text-sm font-bold text-emerald-600 bg-emerald-50 p-2.5 rounded-xl border border-emerald-100 mt-2">
                  <span className="flex items-center"><span className="text-lg mr-2">≡ƒÄë</span> Pre-booking Discount (5%)</span>
                  <span>- Γé╣{Math.round(calculateTotalPrice() * 0.05)}</span>
                </div>
              )}
          ...
          >
            <span className="flex items-center space-x-2">
              ...
              {!loading && calculateTotalItems() > 0 && (
                <>
                  <span className="opacity-50 mx-1">ΓÇó</span>
                  {orderType === 'prebook' ? (
                    <div className="flex items-center space-x-2">
                      <span className="line-through opacity-60 text-sm">Γé╣{calculateTotalPrice()}</span>
                      <span className="text-emerald-400 font-black">Γé╣{calculateTotalPrice() - Math.round(calculateTotalPrice() * 0.05)}</span>
                    </div>
                  ) : orderType === 'takeaway' ? (
                    <span className="text-red-300 font-black">Γé╣{calculateTotalPrice()}</span>
                  ) : (
                    <span>Γé╣{calculateTotalPrice()}</span>
                  )}
                </>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col animate-in slide-in-from-bottom-8 duration-300">
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              ...
              <button onClick={() => setShowHistory(false)} className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center font-bold hover:bg-gray-300 transition-colors">Γ£ò</button>
            </div>
