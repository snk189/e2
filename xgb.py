...
def prepare_data(df):
    ...
    df['prev_qty'] = df['quantity'].shift(1)
    df['rolling_mean_3'] = df['quantity'].rolling(3).mean()
    df['rolling_mean_7'] = df['quantity'].rolling(7).mean()
    df['lag_7'] = df['quantity'].shift(7)

    df[['prev_qty', 'rolling_mean_3', 'rolling_mean_7', 'lag_7']] = \
        df[['prev_qty', 'rolling_mean_3', 'rolling_mean_7', 'lag_7']].fillna(0)
    ...
    # 1. Item-wise previous quantity
    df['item_prev_qty'] = df.groupby('item')['quantity'].shift(1)

    # 2. Item rolling mean (short trend)
    df['item_roll_3'] = df.groupby('item')['quantity'].rolling(3).mean().reset_index(level=0, drop=True)

    # 3. User average quantity
    user_avg = df.groupby('username')['quantity'].mean()
    df['user_avg_qty'] = df['username'].map(user_avg)

    # 4. Interaction feature
    df['weekend_lunch'] = df['is_weekend'] * df['is_lunch']

    # 5. Momentum
    df['momentum'] = df['rolling_mean_3'] - df['rolling_mean_7']

    df[['item_prev_qty','item_roll_3','user_avg_qty','weekend_lunch','momentum']] = \
        df[['item_prev_qty','item_roll_3','user_avg_qty','weekend_lunch','momentum']].fillna(0)
    ...
    df['item_time_avg_qty'] = df.apply(
        lambda row: item_overall_avg[row['item']] if pd.isna(row['item_time_avg_qty']) else row['item_time_avg_qty'], axis=1)
    ...
    df['item_prebook_avg_qty'] = df.apply(
        lambda row: item_overall_avg[row['item']] if pd.isna(row['item_prebook_avg_qty']) else row['item_prebook_avg_qty'], axis=1)
    ...
    df['item_recent_trend'] = df.apply(
        lambda row: item_overall_avg[row['item']] if pd.isna(row['item_recent_trend']) else row['item_recent_trend'], axis=1)
    ...
    feature_cols = [
        ...
        'month_sin','month_cos',
        'prev_qty','rolling_mean_3','rolling_mean_7','lag_7',
