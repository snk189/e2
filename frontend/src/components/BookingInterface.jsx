  ...
  return (
    <div className="app-bg font-body-lg text-on-surface overflow-hidden min-h-dvh relative touch-none">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div 
           className="bg-orb transition-all duration-700"
           id="orb-primary" 
            ...
            <div className="px-margin-mobile space-y-6">
                    ...
                    <div className="sticky top-0 z-40 bg-background/90 backdrop-blur-md border-b border-surface-container-high chip-nav no-scrollbar -mx-margin-mobile mb-4">
                        {categories.map((category) => (
                           <a key={category} className="flex-shrink-0 px-4 py-3 text-[12px] font-bold uppercase tracking-wider active:scale-95 transition-all relative group" 
                              ...
                              href={`#cat-${categoryStyles[category]?.id || category.toLowerCase().replace(/\s+/g, "-")}`}
                           >
                               {category}
                               <div className={`absolute bottom-0 left-4 right-4 h-0.5 rounded-full opacity-80 ultra-pro-header gradient-${categoryStyles[category]?.tone || "main"}`}></div>
                           </a>
                        ))}
                    </div>
                </div>
                ...
                {groupedMenu.map(({ category, items }) => (
                    <React.Fragment key={category}>
                        <div className="category-line" id={`cat-${categoryStyles[category]?.id || category.toLowerCase().replace(/\s+/g, "-")}`}>
                            <span className="font-display-lg text-2xl text-on-surface transition-all duration-300">
                    Schedule Meal
                </h3>
                <p className="text-sm text-on-surface-variant max-w-[240px] transition-all duration-300" style={{transitionDelay: '50ms'}}>
      ...
      <div className="fixed bottom-24 left-margin-mobile right-margin-mobile z-[60] transition-all duration-400 transform">
          <div ref={containerRef} className="relative w-full h-14 bg-white/90 backdrop-blur rounded-full border border-outline-variant/20 flex items-center p-1.5 overflow-hidden shadow-2xl">
              ...
              ></div>
              <div 
                  className="relative z-20 w-11 h-11 rounded-full flex items-center justify-center text-on-secondary"
                  style={{ transform: `translate3d(${slideX}px, 0, 0)`, transition: isDragging ? "none" : "transform 0.3s cubic-bezier(0.18, 0.89, 0.32, 1.28)" }}
      ...
      <nav className="fixed bottom-0 left-0 w-full z-50 bg-white/90 backdrop-blur-xl border-t border-surface-container-high h-20 flex justify-around items-center px-margin-mobile">
        <a className="flex flex-col items-center justify-center text-primary rounded-xl p-2 active:scale-90 transition-transform" href="#" onClick={(e) => {e.preventDefault(); toggleMethod('dine');}}>
            <img src={iconImg} alt="Orders" className={`w-6 h-6 object-contain ${activeTab === 'orders' ? 'brightness-0 invert' : 'brightness-0 opacity-50'}`} style={activeTab === 'orders' ? {filter: 'invert(9%) sepia(85%) saturate(7181%) hue-rotate(345deg) brightness(85%) contrast(105%)'} : {}} />
